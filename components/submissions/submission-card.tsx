...
export function SubmissionCard({ submission }: SubmissionCardProps) {
  const campaignTitle =
    submission.campaigns?.title || submission.campaign?.title || "Campaign";
  ...
  return (
    <div className="flex items-center gap-4 rounded-lg border border-zinc-800 bg-zinc-900/50 p-3">
      ...
      <div className="min-w-0 flex-1">
        <p className="truncate text-sm font-medium text-white">{campaignTitle}</p>
        <div className="mt-0.5 flex items-center gap-3 text-xs text-zinc-500">
          ...
          {(submission.points_awarded ?? 0) > 0 && (
            <span className="flex items-center gap-1 text-emerald-400">
              <Star className="h-3 w-3" />+{submission.points_awarded} pts
            </span>
          )}
        </div>
      </div>
